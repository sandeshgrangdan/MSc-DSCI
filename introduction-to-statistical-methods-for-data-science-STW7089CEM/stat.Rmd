...
```{r}
df <- read.csv( file = file.path(dataset_base_path, "dataset.csv"), header = F, skip = 1)
...
#importing targeted output Y
Y <- df[,c("x2")]
Y <- as.matrix(Y)
cat("Number of rows (Y): ",nrow(Y), "\n")
...
#plotting timeseries data and box plot of target variable
```{r}
generate_ts_box_plot <- function(my_x, my_y) {
  cat("Number of rows (X): ",nrow(my_x), "\n")
  cat("Number of rows (Y): ",nrow(my_y), "\n")
  
  my_time = as.matrix(1:length(Y))
  my_x.ts<-ts(X,start = min(my_time),end = max(my_time),frequency =1)
  my_y.ts<-ts(Y,start = min(my_time),end = max(my_time),frequency =1)
  
  plot(my_x.ts,main = "Time series plot of X Signal", xlab = "Time", ylab = "Input signal")
  plot(my_y.ts,main = "Time series plot of Y Signal", xlab = "Time", ylab = "Output signal")
  ...
  boxplot(y, main = "Boxplot of y (X2)", ylab = "Values")
}

generate_ts_box_plot(X,Y)

```
# Removing outliers and analyzing time series and box plot
```{r}
...
#combining Histogram of X signal with density plot
hist(X,freq = FALSE,main = "Histogram and density plot of X")
...
#Creating a density plot of output signal X2 (Y)
...
#likehood_Model_1
cat("Model1 ::Variance =>",Variance_model1, "::Likelihood =>", likehood_Model_1,"\n")
...
#likehood_Model_2
cat("Model2 ::Variance =>",Variance_model2, "::Likelihood =>", likehood_Model_2,"\n")
...
#likehood_Model_3
cat("Model3 ::Variance =>",Variance_model3, "::Likelihood =>", likehood_Model_3,"\n")
...
#likehood_Model_4
cat("Model4 ::Variance =>",Variance_model4, "::Likelihood =>", likehood_Model_4,"\n")
...
#likehood_Model_5
cat("Model5 ::Variance =>",Variance_model5, "::Likelihood =>", likehood_Model_5,"\n")
...
points(Y_testing_data, y, col = rgb(1, 0, 0, 0.6), pch = 4)

# Add legend
legend("topright", legend = c("Y_testing_hat(Prediction)", "95% CI", "Y_testing_data(Actual)"),
       col = c(rgb(1, 0, 1, 0.6), rgb(1, 0, 0, 1), rgb(0, 0, 0, 0.6)),
       pch = c(19, NA, 4), lty = c(1, NA, NA))
...
plot(f_value_r,s_value_r, col = c("brown", "blue"), main = "Joint and Marginal Posterior Distribution (Rejection)")
par(mfrow=c(1,1))
...
###ploting Joint and Marginal Posterior Distribution of the graph
plot(f_value,s_value, col = c("brown", "blue"), main = "Joint and Marginal Posterior Distribution (Acceptance)")
par(mfrow=c(1,1))
```
